#!/usr/bin/env python3
"""
Create query file for a city in GitHub Actions.
"""
import sys
import math
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 gha_create_queries.py <city_key> <mode>")
        sys.exit(1)
    ...
    print(f'Created {len(cells)} cells × {len(queries)} queries = {len(cells)*len(queries)} lines')
